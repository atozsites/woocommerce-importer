<?php 
    ...
    if(isset($post_data['uploaded_file_path'])) {
        ...
        if ( $handle !== FALSE ) {
            ...
            $error_messages[] = __( 'Could not open CSV file.', 'atoz-woocommerce-product-importer' );
        }

        if(sizeof($import_data) == 0) {
            $error_messages[] = __( 'No data found in CSV file.', 'atoz-woocommerce-product-importer' );
        }
        ...
        foreach($import_data as $row_id => $row) {
            ...
            if(strlen($new_post['post_title']) > 0 || $existing_product !== null) {
                ...
                if($existing_product !== null) {
                    $new_post_messages[] = sprintf( __( 'Updating product with ID %s.', 'atoz-woocommerce-product-importer' ), $existing_product->ID );
                ...
                if(is_wp_error($new_post_id)) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t insert product with name %s.', 'atoz-woocommerce-product-importer' ), $new_post['post_title'] );
                } elseif($new_post_id == 0) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t update product with ID %s.', 'atoz-woocommerce-product-importer' ), $new_post['ID'] );
                } else {
                    //insert successful!
                    ...
                    foreach($new_post_image_urls as $image_index => $image_url) {
                        ...
                        if(!in_array($image_ext, $allowed_extensions)) {
                            $new_post_errors[] = sprintf( __( 'A valid file extension wasn\'t found in %s. Extension found was %s. Allowed extensions are: %s.', 'atoz-woocommerce-product-importer' ), $image_url, $image_ext, implode( ',', $allowed_extensions ) );
                            continue;
                        }
                        ...
                        if(ini_get('allow_url_fopen')) {
                            ...
                            if( ! @copy($image_url, $dest_path)) {
                                ...
                                $new_post_errors[] = sprintf( __( '%s encountered while attempting to download %s', 'atoz-woocommerce-product-importer' ), $http_status, $image_url );
                            }
                            ...
                            if($http_status != 200) {
                                unlink($dest_path);
                                $new_post_errors[] = sprintf( __( 'HTTP status %s encountered while attempting to download %s', 'atoz-woocommerce-product-importer' ), $http_status, $image_url );
                            }
                        } else {
                            //well, damn. no joy, as they say.
                            $error_messages[] = sprintf( __( 'Looks like %s is off and %s is not enabled. No images were imported.', 'atoz-woocommerce-product-importer' ), '<code>allow_url_fopen</code>', '<code>cURL</code>'  );
                            break;
                        }

                        //make sure we actually got the file.
                        if(!file_exists($dest_path)) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t download file %s.', 'atoz-woocommerce-product-importer' ), $image_url );
                            continue;
                        }
                    ...
                    foreach($new_post_image_paths as $image_index => $dest_path_info) {
                        ...
                        if($existing_product !== null && intval($post_data['product_image_skip_duplicates'][$key]) == 1) {
                            ...
                            if(is_array($existing_attachments) && sizeof($existing_attachments) > 0) {
                                ...
                                $new_post_messages[] = sprintf( __( 'Skipping import of duplicate image %s.', 'atoz-woocommerce-product-importer' ), $dest_path_info['source'] );
                                continue;
                            }
                        }
                        ...
                        if(!file_exists($dest_path_info['path'])) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t find local file %s.', 'atoz-woocommerce-product-importer' ), $dest_path_info['path'] );
                            continue;
                        }
                ...
                $new_post_errors[] = __( 'Skipped import of product without a name', 'atoz-woocommerce-product-importer' );
            }
